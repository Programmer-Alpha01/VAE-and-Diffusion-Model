...
class SamplingConfig:
    ...
    output_dir       = 'Diffusion_model/DDPM_MNIST/results'
    num_samples      = 8           # how many images to generate (keep small for GIF)
    grid_size        = 8            # for 8×8 grid = 64 images
    timesteps        = 1000
    beta_start       = 1e-4
    beta_end         = 0.02
...
@torch.no_grad()
def sample_with_gif(model, reverse_process, num_images, device):
    model.eval()
    ...
    # We collect frames from t = T-1 down to t=0
    for t in tqdm(range(cfg.timesteps - 1, -1, -1), desc="Sampling", unit="step"):
        ...
        # Save frame every N steps + always save last frame
        if t % cfg.gif_every == 0 or t == 0:
            # Use final x (x_prev) for visualization
